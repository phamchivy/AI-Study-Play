...
from utils.quiz import QuizManager_for_finger
from functions.finger_counting.count_finger import detect_finger_count
from db.save_db import save_finger_result_to_db
...
# Nội dung hướng dẫn
instructions = [
    "This mode makes use of hand gesture recognition:",
    "- Players can raise 1 to 10 fingers in front of the camera to represent their answer to each math problem.",
    "- The game will detect the number of fingers shown and use it as the player's answer.",
    "- It’s a fun, physical way for young children to interact with numbers and develop basic arithmetic skills through body movement."
]

def show_finger_mode(screen):
    # Khởi tạo QuizManager
    quiz_manager = QuizManager_for_finger()
    feedback_message = ""
    feedback_timer = 0
    color=""
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if start_button.is_clicked(event):
                ...
                user_input = detect_finger_count() 
        ...
        if quiz_manager.is_finished():
            ...
            save_finger_result_to_db(user_name, score, datetime.fromtimestamp(start_time), end_time)
            print("\nĐã lưu kết quả finger counting.")
...
def show_high_scores(screen, badge_images=None):
    scores = fetch_finger_high_scores()  # Lấy điểm từ database
...
def show_finger_selection(screen):
    while True:
        draw_gradient(screen, (173, 216, 230), (255, 240, 245))  # Background gradient
        update_particles(screen)  # Hiển thị các particle effects (nếu có)
