...
def show_number(screen,y_pos,instructions,SCREEN_WIDTH):
    text_path="../assets/fonts/Gamefont.ttf"
    text_font=pygame.font.Font(text_path, 60)
    NAVY = (25, 25, 112)
    for i, instruction  in enumerate(instructions):
        wrapped_lines = wrap_text(instruction, text_font, SCREEN_WIDTH - SCREEN_WIDTH // 5.5)
    # Vẽ từng dòng sau khi đã chia
        for line in wrapped_lines:
            ...
            # Căn chỉnh văn bản theo vị trí ngang và dọc
            x_pos = SCREEN_WIDTH // 6 # Căn từ SCREEN_WIDTH // 6
            y_pos += 70        # Cách nhau 50px cho mỗi dòng
            screen.blit(text_surface, (x_pos, y_pos))

def show_feedback(screen,y_pos,instructions,SCREEN_WIDTH,color):
    text_path="../assets/fonts/Text.ttf"
    text_font=pygame.font.Font(text_path, 50)
    for i, instruction  in enumerate(instructions):
        wrapped_lines = wrap_text(instruction, text_font, SCREEN_WIDTH - SCREEN_WIDTH // 5.5)
    # Vẽ từng dòng sau khi đã chia
        for line in wrapped_lines:
            ...
            # Căn chỉnh văn bản theo vị trí ngang và dọc
            x_pos = SCREEN_WIDTH // 1.5 # Căn từ SCREEN_WIDTH // 6
            y_pos += 50        # Cách nhau 50px cho mỗi dòng
            screen.blit(text_surface, (x_pos, y_pos))